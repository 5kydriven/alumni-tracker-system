    ...
    <form @submit.prevent="addAlumni(data)">
        ...
        <div class="grid gap-2 mb-4 sm:grid-cols-3">
            ...
            <div v-show="data.employment.status == 'Employed'">
                ...
                <InputText type="text" :disabled="!data.employment.type"
                    v-if="data.employment.type == 'Formally Employed'" v-model="data.employment.company"
                    :invalid="invalid.employment.company" />
...
const addAlumni = async (cred) => {
    ...
    try {
        const response = await fetch('http://localhost:8000/createUser', {
            ...
            body: JSON.stringify({
                email: cred.studentId + "@gmail.com",
                password: "alumni123",
            }),
        });
        if (!response.ok) {
        ...
        await setDoc(doc(db, "accountRoles", data.uid), { role: 'alumni' });
        await setDoc(doc(db, "alumni", data.uid), { ...cred, fullname: cred.firstname + " " + cred.middlename + " " + cred.lastname, dateAdded: serverTimestamp(), });
